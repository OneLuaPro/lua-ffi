...
static bool cdata_from_lua_table(lua_State *L, struct ctype *ct, void *ptr, int idx, bool cast)
{
    ...
    if (ct->type == CTYPE_ARRAY) {
        ...
        while (i < ct->rc->nfield) {
            ...
            if (!lua_isnil(L, -1)) {
                if (field->bit_size)
                    cdata_from_lua_bitfield(L, field, (char *)ptr + field->offset, lua_absindex(L, -1));
                else
...
static int cdata_index_crecord(lua_State *L, struct cdata *cd, struct ctype *ct, bool to)
{
    ...
    if (to) {
        if (field->bit_size)
            cdata_to_lua_bitfield(L, field, (char *)ptr + offset);
        else
        ...
        if (field->bit_size)
            return cdata_from_lua_bitfield(L, field, (char *)ptr + offset, 3);
...
static int cparse_function_arg(lua_State *L, int tok, struct ctype *ct, char **name)
{
    ...
    if (cparse_check_tok(L, tok) == '(') {
        struct ctype fargs[MAX_FUNC_ARGS] = {0};
        struct ctype fct;
        int fnarg = 0;
        int ptr_depth = 0;
...
{
    struct ctype args[MAX_FUNC_ARGS] = {0};
    struct ctype fct;
    int narg = 0;
    bool va = false;
